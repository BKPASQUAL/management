...
export default function CheckingStatusPage() {
  const [searchTerm, setSearchTerm] = useState<string>("");
  const [selectedRepresentative, setSelectedRepresentative] =
    useState<string>("all");
  const { toast } = useToast();

  // Fetch checking orders
  const {
    data: ordersResponse,
    isLoading,
    error,
  ...
  // Filter to ensure only checking orders are shown (additional safeguard)
  const checkingOrders = useMemo(() => {
    return orders.filter(
      (order) => order.order_status.toLowerCase() === "checking"
    );
  }, [orders]);
  ...
  const filteredOrders = useMemo(() => {
    return checkingOrders.filter((order) => {
      const matchesSearch =
        searchTerm === "" ||
  ...
  }, [checkingOrders, searchTerm, selectedRepresentative]);

  // Action handlers
  const handleApproveOrder = async (orderId: number, invoiceNo: string) => {
    try {
      const response = await fetch(
        `${
          process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3001/"
        }customer-bills/${orderId}/approve`,
        {
          method: "POST",
          headers: {
      ...
      toast({
        title: "Success",
        description: `Order ${invoiceNo} approved successfully`,
      });
      refetch();
    } catch (error: any) {
      toast({
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center py-8">
        <div className="text-lg">Loading checking orders...</div>
      </div>
    );
  }
      ...
      <div className="mb-4 flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm sm:text-base mt-1">
            Review and manage orders under checking
          </p>
        </div>

        {/* Results Count - Desktop */}
        <div className="hidden lg:block border p-3 rounded-lg text-sm">
          Showing {filteredOrders.length} of {checkingOrders.length} orders
        </div>
      </div>
