 ...
 * Handles potential errors if localStorage is not available (e.g., during SSR).
...
export const customerBillApi = createApi({
  ...
  baseQuery: fetchBaseQuery({
    ...
    prepareHeaders: (headers) => {
      headers.set("content-type", "application/json"); // Add authorization header if needed
  ...
  endpoints: (builder) => ({
      ...
      query: (billData) => ({
        ...
        body: {
          ...
          billing_date: billData.billing_date
            ? billData.billing_date.toISOString()
            : undefined,
        },
      }),
    ...
    }), // Fetch all customer bills with optional filters // getCustomerBills: builder.query<CustomerBillListResponse, BillFilterDto | void>({ //   query: (filters = {}) => { //     const params = new URLSearchParams(); //     if (filters.customer_id) params.append('customer_id', filters.customer_id.toString()); //     if (filters.status) params.append('status', filters.status); //     if (filters.from_date) params.append('from_date', filters.from_date.toISOString()); //     if (filters.to_date) params.append('to_date', filters.to_date.toISOString()); //     if (filters.location_id) params.append('location_id', filters.location_id.toString()); //     return `customer-bills?${params.toString()}`; //   }, //   providesTags: ["CustomerBill"], //   transformResponse: (response: CustomerBillListResponse) => { //     console.log("getCustomerBills API Response:", response); //     return response; //   }, //   transformErrorResponse: (response: any) => { //     console.error("getCustomerBills API Error:", response); //     return { //       message: //         response?.data?.message || //         response?.message || //         "Failed to fetch customer bills", //       status: response?.status || 500, //       data: response?.data || null, //     }; //   }, // }), // // Get single customer bill by ID // getCustomerBillById: builder.query<CustomerBillResponse, number>({ //   query: (id) => `customer-bills/${id}`, //   providesTags: (result, error, id) => [{ type: "CustomerBill", id }], //   transformResponse: (response: CustomerBillResponse) => { //     console.log("getCustomerBillById API Response:", response); //     return response; //   }, //   transformErrorResponse: (response: any) => { //     console.error("getCustomerBillById API Error:", response); //     return { //       message: //         response?.data?.message || //         response?.message || //         "Failed to fetch customer bill", //       status: response?.status || 500, //       data: response?.data || null, //     }; //   }, // }), // Update customer bill

    updateCustomerBill: builder.mutation<
      CustomerBillResponse,
      { id: number; data: UpdateCustomerBillDto }
    >({
      query: ({ id, data }) => ({
    ...
    updateBillPayment: builder.mutation<
      CustomerBillResponse,
      { id: number; payment: UpdatePaymentDto }
    >({
      query: ({ id, payment }) => ({
    ...
    getBillSummary: builder.query<
      BillSummaryResponse,
      { customer_id?: number; from_date?: Date; to_date?: Date }
    >({
      query: (params = {}) => {
